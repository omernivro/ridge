...
class ridge_data(object):
    ...
    def trval_te_idx_split(self):
        ...
        self.x_trval, self.y_trval = self.x[val_idx], self.y[val_idx]
        self.x_te, self.y_te = self.x[val_idx], self.y[val_idx]
    ...
    def split_cv(self, iter_num, trval_idx):
        ...
        val_idx = trval_idx[val_st_idx: val_end_idx]
        tr_idx = trval_idx[~np.isin(trval_idx, val_idx)]

        self.x_val, self.y_val = self.x[val_idx], self.y[val_idx]
        self.x_tr, self.y_tr = self.x[tr_idx], self.y[tr_idx]

        return(self.x_val, self.y_val, self.x_tr, self.y_tr)
...
def main():
    ...
    a, b = rid_data.trval_te_idx_split()
    # print a, b
    g, h, i, j = rid_data.split_cv(2, a)
    print h
